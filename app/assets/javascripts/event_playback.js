(function ( $, window, document, undefined ) {
  ...
  $.stopPlayEvents = function (options, callback) {
    if (callback) {
      options.finished = callback;
    }
    return (new EventPlayer(options)).stop();
  };
  ...
  EventRecorder = function (options) {
    this.frames = [];
    Cookies.set('analytics-js', this.frames);
    this.options = $.extend({
      dom: $(document),
      delay: 500
    }, options || {});
  ...
  EventPlayer.prototype = {
    ...
    showDisc: function (color) {
      this.disc.addClass(color || "blue").fadeIn(100);
      return this;
    },
    /**
    ...
    displayInputCursor: function() {
      this.cursor = $("<div class=\"input-cursor\"><\/div>");
      this.disc = $(".disc", this.cursor);
      this.options.dom.prepend(this.cursor);
      this.cursor.fadeIn("slow");
      return this;
